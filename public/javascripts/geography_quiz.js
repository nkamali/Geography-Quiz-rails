...
Originally this Geography Quiz was written with a php backend and was migrated to Rails 3 on 4/14/2011
*/
...
function giveUp(indexOfRadioButtonClickedOn, parent)
{
	...
	{
		var itemText = list.find('item').text();
		if(itemText == '') {
			msg = msg + " is: NO results matched this criteria for this country";
		}	
		else {
...
{	
	var url = findURL(indexOfRadioButtonClickedOn);

	// POST the XMLHttpRequest via JQuery
	$.post(url, { country: randomCountryChosen },
	function(data){		
...
// Finds the url from quiz_types array in quiz.json file
function findURL(indexOfRadioButtonClickedOn)
{
	return quizTypes[indexOfRadioButtonClickedOn].url;
}
...
{
	// Gets the url which allows you to get the surface area of the specified country
	var surfaceAreaURLFromJSON = SurfaceArea.url;
	...
	$.post(surfaceAreaURLFromJSON, { country: randomCountryChosen },
	function(data){	
